...
public class StockExchange {
    static volatile StockExchange instance;
    final Map<String, List<Order>> buyOrders;
    final Map<String, List<Order>> sellOrders;
    ...
    public Order findBestBuy(List<Order> buys)
    {
        return buys.stream().filter(o -> o.getStatus() == OrderStatus.OPEN)
                .max(Comparator.comparingDouble(Order::getPrice))
                .orElse(null);
    }

    public Order findBestSell(List<Order> sells)
    {
        return sells.stream().filter(o -> o.getStatus() == OrderStatus.OPEN)
                .max(Comparator.comparingDouble(Order::getPrice))
                .orElse(null);
    }
}
