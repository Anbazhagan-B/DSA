package com.DsaProject.LLD.SplitWiseLLD;
...
public class SplitwiseService {
    ...
    public List<Transaction> simplifyGroupDebts(String groupid) {
        ...
        List<Map.Entry<User, Double>> debitors = netBalances.entrySet().stream().filter(e -> e.getValue() < 0).collect(Collectors.toList());
        ...
        while (i < creditors.size() && j < debitors.size()) {
            ...
            Map.Entry<User, Double> debtor = debitors.get(i);
